...
import { trafficSegments, type Corridor, type CorridorStatus } from './corridors';
...
function formatDelay(value: number | null) {
  return value === null ? 'Geen data' : value === 0 ? 'Vrij' : `+${value} min`;
}

function statusLabel(status: CorridorStatus) {
  return status === 'groen' ? 'Vrij verkeer' : status === 'oranje' ? 'Druk' : status === 'rood' ? 'Vertraging' : 'Geen data';
}
...
function NdwRoadLayer() {
  ...
  >
    <Tooltip sticky><strong>{segment.name}</strong><br/>{statusLabel(segment.status)}</Tooltip>
    <Popup>
      <div className="trafficPopup roadPopup">
        <b>{segment.name}</b>
        <small>{segment.subtitle}</small>
        <strong className={`roadDelay ${segment.status}`}>{statusLabel(segment.status)}</strong>
        {segment.speedKmh !== null && <span>{segment.speedKmh} km/u</span>}
        {segment.lengthMeters && <small>Meetvak: {segment.lengthMeters} meter</small>}
        <small>Bron: NDW · laatste meting</small>
      </div>
    </Popup>
  </Polyline>)}</>;
}
...
function SwitchLaneLayer() {
  const lane = trafficSegments.find(segment => segment.switchLane);
  if (!lane) return null;
  const now = new Date();
  const parts = new Intl.DateTimeFormat('nl-NL', { timeZone: 'Europe/Amsterdam', hour: '2-digit', minute: '2-digit', hour12: false }).formatToParts(now);
  const hour = Number(parts.find(part => part.type === 'hour')?.value || 0);
  const minute = Number(parts.find(part => part.type === 'minute')?.value || 0);
  const open = hour * 60 + minute >= 20 * 60 || hour * 60 + minute < 14 * 60;
  return <Polyline positions={lane.path} pathOptions={{ color: open ? '#2577d8' : '#7b8580', weight: 6, opacity: .9, dashArray: '10 8' }}>
    <Tooltip sticky><strong>Wisselstrook</strong><br/>{open ? 'Open richting Krimpen uit' : 'Gesloten richting Krimpen uit'}</Tooltip>
    <Popup><div className="trafficPopup roadPopup"><b>Wisselstrook</b><strong>{open ? 'Open' : 'Gesloten'}</strong><span>{open ? 'Richting Krimpen uit' : 'Opent om 20:00'}</span><small>Alleen via Nieuwe Tiendweg · maximaal 1,80 m</small></div></Popup>
  </Polyline>;
}
